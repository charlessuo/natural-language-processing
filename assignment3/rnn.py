...
class RNN:
    def __init__(self, vocab_size, num_classes, sentence_len, embed_size=100, cell_size=128, num_layers=1):
...
#            W = tf.Variable(tf.truncated_normal([2 * cell_size, num_classes], stddev=1.0 / np.sqrt(num_classes)), name='W')
            ...
            #pred_mask = tf.reshape(tf.sign(inputs_flat), shape=[-1, sentence_len]) # (N, sentence_len)
    ...
    def generate_submission(preds, id_to_class, filename='submission'):
        seq_len = self.sess.run(self.seq_len, feed_dict={self.inputs: x})
        with open('./results/' + filename + '.csv', 'w') as f:
            f.write('id,tag\n')
            for i in range(len(preds)):
                f.write('{},{}\n'.format(str(i), id_to_class[preds[i]]))
...
if __name__ == '__main__':
    ...
    rnn = RNN(len(id_to_word), len(id_to_class), max_len, embed_size=100, cell_size=256, num_layers=2)
    rnn.train(train_x, train_y, num_epoch=2, batch_size=64)
