...
#            self.rnn_outputs_flat = tf.reshape(self.rnn_output, shape=[-1, 2 * cell_size])
            output, _ = tf.nn.dynamic_rnn(cell, self.embedded_x, dtype=tf.float32)
            self.rnn_outputs_flat = tf.reshape(output, shape=[-1, cell_size])
        ...
        with tf.name_scope('output'):
            ...
            pred_mask = tf.reshape(tf.sign(labels_flat), shape=[-1, sentence_len])
        ...
        with tf.name_scope('accuracy'):
            num_correct = tf.equal(self.predictions, self.labels)
    ...
    def train(self, train_x, train_y, num_epoch=20, batch_size=32):
        ...
        for batch_x, batch_y in batch_pairs:
            ...
            if step % 5 == 0:
                print('Epoch: {}, loss: {:.6f}, accuracy: {:.2f}'.format(int(step / batches_per_epoch), loss, acc))
                print(self.sess.run(self.predictions, feed_dict={self.inputs: batch_x, self.labels: batch_y}))
                print(self.sess.run(self.seq_len, feed_dict={self.inputs: batch_x, self.labels: batch_y}))
            step += 1
...
if __name__ == '__main__':
    ...
    rnn = RNN(len(id_to_word), len(id_to_class), max_len, embed_size=100, cell_size=128, num_layers=1)
    rnn.train(train_x, train_y, num_epoch=20, batch_size=64)
